...
**Step 2-d: 地球表示（3D）モードの活用**

碁盤の目状に区画整理された住宅街では、真上からの航空写真だけでは建物の区別が難しい。
Google Mapsの「地球表示」（3D）モードを使うと斜めから建物を見ることができ、
外壁の色・形状・階数をSUUMOの外観写真と照合しやすくなる。

1. 地図右下の「レイヤ」から「地球表示」を有効にする（またはマップタイプの切り替えで「Globe」を選ぶ）
2. Ctrl+ドラッグ（またはShift+ドラッグ）で視点を傾けて斜め上から見下ろすアングルにする
3. 物件の接道方向に合わせて視点を回転させる
   - 例えば「東4m幅」道路なら東側から見るアングルにすると、道路側の外観が見える
4. SUUMOの外観写真と建物の外壁色・形状を比較して候補を絞る

このモードは特に以下のケースで有効：
- 区画整理された新しい住宅街で似た形の家が並んでいるとき
- ストリートビューが狭い道に入れず建物が見えないとき
- 接道方向と外壁を同時に確認したいとき
